...
class Nonneg_dagma():
    ...
    def proj_grad_step_W_(self, W, Sigma, alpha, lamb, stepsize, iter):
        self.Gw_obj_func = self.compute_gradient_W_(W, Sigma, lamb, alpha)
    ...
    def acc_proj_grad_desc_(self, W, Sigma, lamb, alpha, stepsize, max_iters, checkpoint, tol,
        ...
        for i in range(max_iters):
            ...
            if self.restart and self.Gw_obj_func.flatten().T @ (diff_W).flatten() > 1e-6:
                W = W_prev.copy()                    
                W_fista = W.copy()
                t_k = 1
                continue
...
class MetMulColide(MetMulDagma):
    ...
    def fit(self, X, lamb, stepsize, s=1, iters_in=1000, iters_out=10, checkpoint=250, tol=1e-6,
            beta=5, gamma=.25, rho_0=1, alpha_0=.1, track_seq=False, dec_step=None,
            beta1=.99, beta2=.999, Sigma=None, scale_sig=.01, delta=.01, sca_adam=False, verb=False):
        
        self.init_variables_(X, rho_0, alpha_0, track_seq, s, Sigma, scale_sig, beta1, beta2, delta, verb)        
    ...
    def init_variables_(self, X, rho_init, alpha_init, track_seq, s, Sigma_init, scale_sig, beta1, beta2, delta, verb):
        # Default initialization of Sigma0        
        if Sigma_init is None:
            Sigma_init = np.linalg.norm(X, axis=0) / np.sqrt(X.shape[0])

        super().init_variables_(X, rho_init, alpha_init, track_seq, s, Sigma_init, beta1, beta2, delta, verb)
